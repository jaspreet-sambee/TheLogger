...
struct WorkoutDetailView: View {
        ...
        workoutList
            ...
            .onAppear {
                ...
                if cachedSuggestions.isEmpty {
                    cachedSuggestions = ExerciseSuggester.suggest(for: workout, modelContext: modelContext, limit: 5)
                }
            }
            .onDisappear {
                stopTimer()
    ...
    private var recentExercisesQuickAdd: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack(spacing: 10) {
                    ForEach(cachedSuggestions, id: \.self) { exerciseName in
                        Button {
                ...
                Menu {
                    ForEach(otherExercises) { other in
                        Button {
                            ...
                            Analytics.send(Analytics.Signal.supersetCreated)
                        } label: {
    ...
    private func addExerciseWithMemory(name: String) -> Exercise {
        Analytics.send(Analytics.Signal.exerciseAdded, parameters: ["exerciseName": name])
                ...
                if autoPopulateSets {
                    ...
                    if let previousExercise = Exercise.findPreviousExercise(
                        ...
                        for previousSet in previousExercise.setsByOrder {
                            if isTimeBased {
                                exercise.addSet(reps: 0, weight: 0, durationSeconds: previousSet.durationSeconds)
                            } else {
                                exercise.addSet(reps: previousSet.reps, weight: previousSet.weight)
                            }
                        }
                    } else {
    ...
    private func endWorkout() {
        ...
        let totalSets = (workout.exercises ?? []).reduce(0) { $0 + ($1.sets?.count ?? 0) }
        ...
        Analytics.send(Analytics.Signal.workoutCompleted, parameters: [
    ...
    private func discardWorkout() {
        Analytics.send(Analytics.Signal.workoutDiscarded)
